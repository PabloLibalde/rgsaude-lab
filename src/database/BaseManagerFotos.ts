...
import DatabaseFotos from './DatabaseFotos';

class BaseManagerFotos extends DatabaseFotos {
  private ibge: string;
  ...
  private getConnection(): Promise<Firebird.Database> {
    return new Promise((resolve, reject) => {
      const [{ database }] = this.databaseFotosPools.filter((dp) => dp.ibge === this.ibge);
...
export default new BaseManagerFotos();
